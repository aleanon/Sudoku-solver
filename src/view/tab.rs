...
use crate::{styles::{self, ThickRule}, sudoku::Sudoku, update::{Message, SudokuSize}};
...
impl<'a> Tab {
    ...
    pub fn view(&'a self) -> iced::Element<'a, Message> {
      let center:Element<'a, Message> = match &self.sudoku {
        Some(sudoku) => {
            ...
            let clear_button_text = if self.solving {"Abort"} else {"Clear"};

            let clear_button = button(text(clear_button_text)
                    .horizontal_alignment(iced::alignment::Horizontal::Center)
                    .vertical_alignment(iced::alignment::Vertical::Center)
                )
                .width(150)
                .height(60)
                .on_press(if self.solving {Message::AbortSolving} else {Message::Clear});
    ...
    fn sudoku_grid(solving: bool, selected: &Option<(usize, usize)>, sudoku: &'a Sudoku) -> Element<'a, Message> {
        let mut rows = column![].push(widget::Rule::horizontal(1)
        ...
        for (rowindex,row) in sudoku.puzzle.iter().enumerate() {
            ...
            for (columnindex, &field) in row.iter().enumerate() {
                ...
                for (x, y) in &sudoku.conflicts {
                    if rowindex == *x && columnindex == *y {
                        if is_selected {
                            tile = tile.style(theme::Button::custom(styles::NumberColitionSelected))
                        } else {
            ...
            if (rowindex + 1) % (sudoku.size as f32).sqrt() as usize == 0 {
                ...
                rows = rows.push(row_widget).push(widget::Rule::horizontal(1))
            }
        }
        rows.into()
    }
